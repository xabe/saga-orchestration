...
class PaymentEventConsumerTest {
  ...
  public void givenAEventValidCanceledWhenInvokeTryPublishThenSendEvent() throws Exception {
    //Given
    final PaymentCancelCommandEvent event = PaymentMother.createPaymentCancelCommandEvent();
    final ArgumentCaptor<PaymentCanceledEvent> argumentCaptor = ArgumentCaptor.forClass(PaymentCanceledEvent.class);
    doAnswer(invocationOnMock -> {
      final Payment payment = Payment.class.cast(invocationOnMock.getArguments()[1]).toBuilder().id(1L).price(10L).build();
      return Uni.createFrom().item(payment);
    }).when(this.paymentRepository).update(any(), any());
    ...
    //Then
    verify(this.eventPublisher).tryPublish(argumentCaptor.capture());
    final PaymentCanceledEvent result = argumentCaptor.getValue();
    assertThat(result, is(notNullValue()));
    assertThat(result.getId(), is(1L));
    assertThat(result.getPurchaseId(), is(event.getPurchaseId()));
    assertThat(result.getUserId(), is(event.getUserId()));
  ...
  public void givenAEventValidCancelWhenInvokeTryPublishThenSendEventError() throws Exception {
    ...
    final PaymentCancelCommandEvent event = PaymentMother.createPaymentCancelCommandEvent();
    final ArgumentCaptor<PaymentCanceledEvent> argumentCaptor = ArgumentCaptor.forClass(PaymentCanceledEvent.class);
    when(this.paymentRepository.update(any(), any())).thenReturn(Uni.createFrom().failure(new RuntimeException()));
    ...
    final PaymentCanceledEvent result = argumentCaptor.getValue();
    assertThat(result, is(notNullValue()));
    assertThat(result.getId(), is(event.getPaymentId()));
    assertThat(result.getPurchaseId(), is(event.getPurchaseId()));
    assertThat(result.getUserId(), is(event.getUserId()));
    assertThat(result.getProductId(), is(event.getProductId()));
    assertThat(result.getCreatedAt(), is(event.getSentAt()));
