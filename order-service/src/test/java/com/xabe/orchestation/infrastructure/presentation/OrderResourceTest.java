...
import com.xabe.orchestation.infrastructure.presentation.mapper.OrderPayloadMapperImpl;
...
class OrderResourceTest {
  ...
  public void setUp() throws Exception {
    ...
    this.orderResource = new OrderResource(this.orderUseCase, new OrderPayloadMapperImpl());
  }
  ...
  public void shouldCreatedOrder() throws Exception {
    //Given
    final OrderPayload orderPayload = OrderPayload.builder().productId("1").userId("2").build();
    final Order order = Order.builder().productId("1").userId("2").build();
  ...
  public void shouldCreatedOrderError() throws Exception {
    ...
    final UriInfo uriInfo = mock(UriInfo.class);
    final UriBuilder uriBuilder = new UriBuilderImpl();
    when(this.orderUseCase.create(eq(order))).thenReturn(Uni.createFrom().failure(RuntimeException::new));
    when(uriInfo.getRequestUriBuilder()).thenReturn(uriBuilder);
