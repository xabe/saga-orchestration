...
public class EventConsumer {
  ...
  @Incoming("orders")
  public CompletionStage<Void> consumeKafka(final IncomingKafkaRecord<String, MessageEnvelopeOrder> message) {
    final Metadata metadata = message.getMetadata();
    this.logger.info("Received a message. message: {} metadata {}", message, metadata);
    final MessageEnvelopeOrder messageEnvelopeOrder = message.getPayload();
