...
class EventConsumerTest {
  ...
  public void shouldConsumeEvent() throws Exception {
    ...
    final MessageEnvelopeStatus messageEnvelopeOrder =
        MessageEnvelopeStatus.newBuilder().setMetadata(this.createMetaData()).setPayload(orderCreateCommand)
            .build();
    final ConsumerRecord<String, MessageEnvelopeStatus> consumerRecord = new ConsumerRecord<>("topic", 1, 1L, "key", messageEnvelopeOrder);
    ...
    final IncomingKafkaRecord<String, MessageEnvelopeStatus> incomingKafkaRecord =
  ...
  public void notShouldHandlerEvent() throws Exception {
    final MessageEnvelopeStatus messageEnvelopeOrder =
        MessageEnvelopeStatus.newBuilder().setMetadata(this.createMetaData()).setPayload(mock(SpecificRecord.class))
            .build();
    final ConsumerRecord<String, MessageEnvelopeStatus> consumerRecord = new ConsumerRecord<>("topic", 1, 1L, "key", messageEnvelopeOrder);
    final KafkaCommitHandler kafkaCommitHandler = new KafkaIgnoreCommit();
    final KafkaFailureHandler kafkaFailureHandler = new KafkaIgnoreFailure("channel");
