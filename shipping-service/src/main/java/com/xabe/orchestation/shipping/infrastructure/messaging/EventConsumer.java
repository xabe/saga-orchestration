...
public class EventConsumer {
  ...
  @Incoming("shipments")
  public CompletionStage<Void> consumeKafka(final IncomingKafkaRecord<String, MessageEnvelopeShipping> message) {
    final Metadata metadata = message.getMetadata();
    this.logger.info("Received a message. message: {} metadata {}", message, metadata);
    final MessageEnvelopeShipping messageEnvelopeShipping = message.getPayload();
    final Class<?> msgClass = messageEnvelopeShipping.getPayload().getClass();
    final SpecificRecord payload = SpecificRecord.class.cast(messageEnvelopeShipping.getPayload());
