package com.xabe.orchestration.shipping.infrastructure.integration;
...
public class SagaEventTest {
  ...
  public void shouldCancelOrderSaga() throws Exception {
    //Given
    final String purchaseId = "1111";
    final ShippingCancelCommand shippingCancelCommand =
        ShippingCancelCommand.newBuilder().setShippingId(this.shippingId).setPurchaseId(purchaseId).setProductId("1").setUserId("2")
            .setSentAt(Instant.now())
            .build();
    final MessageEnvelopeShipping messageEnvelopeShipping = MessageEnvelopeShipping.newBuilder()
        .setMetadata(this.createMetaData())
        .setPayload(shippingCancelCommand)
        .build();
    ...
    Awaitility.await().pollDelay(DELAY_MS, TimeUnit.MILLISECONDS).pollInterval(POLL_INTERVAL_MS, TimeUnit.MILLISECONDS)
        .atMost(TIMEOUT_MS, TimeUnit.MILLISECONDS).until(() -> {
          final Tuple2<String, MessageEnvelopeStatus> result = KAFKA_CONSUMER.expectMessagePipe(ShippingCanceledEvent.class, TIMEOUT_MS);
          assertThat(result, is(notNullValue()));
          assertThat(result.getV1(), is(notNullValue()));
          assertThat(result.getV2(), is(notNullValue()));
          final ShippingCanceledEvent shippingCanceledEvent = ShippingCanceledEvent.class.cast(result.getV2().getPayload());
          assertThat(shippingCanceledEvent.getUpdatedAt(), is(notNullValue()));
          assertThat(shippingCanceledEvent.getOperationStatus(), is(ShippingOperationStatus.SUCCESS));
          final Shipping shipping = shippingCanceledEvent.getShipping();
          assertThat(shipping.getId(), is(notNullValue()));
          assertThat(shipping.getPurchaseId(), is(shippingCancelCommand.getPurchaseId()));
          assertThat(shipping.getProductId(), is(shippingCancelCommand.getProductId()));
          assertThat(shipping.getUserId(), is(shippingCancelCommand.getUserId()));
          assertThat(shipping.getPrice(), is(100L));
          assertThat(shipping.getStatus().name(), is("CANCELED"));
          assertThat(shipping.getCreatedAt(), is(notNullValue()));
          return true;
        });
  }
